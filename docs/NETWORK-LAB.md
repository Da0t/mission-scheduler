# UDP learning lab (secondary application)
...
Install JDK 21 or newer. Double-click **Mission Network Lab.command** on macOS, or run `./mission-scheduler`, then open http://127.0.0.1:8080/network.html. The macOS launcher uses the packaged JAR when available and opens the browser once the server responds. Without a packaged JAR, the first launch downloads Maven and dependencies and may take a few minutes. Run `./mvnw package` after source changes to refresh the packaged app. Stop the server with Ctrl+C in its launcher terminal.
...
Read [the learning guide](LEARNING.md) for Java benefits, networking concepts, exercises, and the next steps toward a stronger portfolio project.
